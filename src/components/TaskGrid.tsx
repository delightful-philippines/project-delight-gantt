...
export const TaskGrid = React.memo(function TaskGrid({
  ...
  const [startIndex, endIndex] = useMemo(() => {
    const buffer = 10;
    const from = Math.max(0, Math.floor(scrollTop / rowHeight) - buffer);
    const to = Math.min(rowIds.length, from + Math.ceil((viewportHeight || 620) / rowHeight) + buffer * 2);
    return [from, to];
  }, [rowHeight, rowIds.length, scrollTop, viewportHeight]);
  ...
  return (
    <section className="min-h-full min-w-0 border-r border-slate-200 bg-white flex flex-col">
      <div className="min-h-0 flex-1">
        <div
          className="sticky top-0 z-100 grid items-center border-b border-slate-200 bg-white/95 backdrop-blur-sm text-xs font-medium uppercase tracking-wider text-slate-400"
          style={{ gridTemplateColumns: gridColumns, width: 'max-content', minWidth: '100%', height: headerHeight }}
        >
        ...
        <div style={{ height: rowIds.length * rowHeight, position: "relative", width: '100%' }}>
          {rowIds.length === 0 && (
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 overflow-hidden px-4" style={{ paddingLeft: `${task.level * 16 + 16}px` }}>
                  ...
                  />
                  <span className="truncate text-sm tracking-tight text-slate-700 font-medium" title={task.title}>
                    {task.title}
                  </span>
                </div>
                  ...
                  {(() => {
                    ...
                    return (
                      <div
                        ...
                        title={`${formattedName}${assignee ? ` (${assignee})` : ''}`}
                      >
                        <UserAvatar 
                ...
                <div className={`sticky right-0 h-full border-l border-slate-50 bg-inherit px-2 ${isSelected ? "z-95" : "z-80"}`}>
                  <div className="flex h-full items-center justify-center gap-1 relative">
                    <div className={`hidden sm:flex items-center gap-1 transition-all duration-200 ${isSelected ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
                      <button
                        type="button"
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        className="inline-flex h-7 w-7 items-center justify-center rounded-xl text-slate-400 hover:bg-blue-50 hover:text-blue-600 transition-all"
                        title="Add Subtask"
                      >
                    ...
                    {isSelected && rect && createPortal(
                      ...
                      >
                        {[
                          ...
                          { label: "Duplicate", icon: <path d="M8 7h8m-8 5h8m-8 5h8M5 7h.01M5 12h.01M5 17h.01" />, onClick: () => onDuplicateTask(task.id) },
